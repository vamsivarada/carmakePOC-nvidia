...
class LPRDetector:
    ...
    def get_video_fps(self):
        """Get the FPS of the video for timestamp calculation"""
        try:
                                ...
                                try:
                                    ...
                                    if plate_text and plate_text.strip():
                                        ...
                                        if plate not in self.plate_detections:
                                            ...
                                            self.plate_detections[plate]["last_seen_frame"] = self.frame_count
                                            self.plate_detections[plate]["last_seen_timestamp"] = timestamp
                                            self.plate_detections[plate]["all_frames"].append(self.frame_count)
                                            self.plate_detections[plate]["all_timestamps"].append(timestamp)
                                            
                                            # Update max confidence if higher
                                            if label_info.result_prob > self.plate_detections[plate]["confidence"]:
                                                self.plate_detections[plate]["confidence"] = label_info.result_prob
    ...
    def save_results(self):
        ...
        for plate, info in valid_plate_detections.items():
            # Calculate duration vehicle was visible
            duration_frames = info["last_seen_frame"] - info["first_seen_frame"]
            duration_seconds = duration_frames / self.video_fps
            ...
            plate_summary = {
                ...
                "last_seen_timestamp": info["last_seen_timestamp"],
                "detection_count": info["count"],
                "duration_frames": duration_frames,
                "duration_seconds": round(duration_seconds, 2),
                "all_frames": info["all_frames"],
                "all_timestamps": info["all_timestamps"],
...
def process_video_ui(video_source, uploaded_video, folder_video, target_plates_text, 
    ...
    try:
        ...
        if not images:
            ...
            status_msg += f"\n### 🚗 Detected Vehicles (with Timeline)\n\n"
            ...
            for plate_info in summary.get('plates', []):
                if plate_info.get('is_valid_uk', True) and plate_info.get('is_target', True):
                    ...
                    # Vehicle header
                    status_msg += f"---\n\n"
                    status_msg += f"**{plate}** | {make} {car_type} | {db_status}\n\n"
                    status_msg += f"- **Location:** {location}\n"
                    status_msg += f"- **Type:** {plate_info.get('vehicle_type', 'Unknown')}\n"
                    status_msg += f"- **Detections:** {plate_info.get('detection_count', 0)}\n"
                    status_msg += f"- **First Seen:** Frame {plate_info.get('first_seen_frame', 0)} @ {plate_info.get('first_seen_timestamp', 'N/A')}\n"
                    status_msg += f"- **Last Seen:** Frame {plate_info.get('last_seen_frame', 0)} @ {plate_info.get('last_seen_timestamp', 'N/A')}\n"
                    status_msg += f"- **Duration:** {plate_info.get('duration_seconds', 0):.1f}s ({plate_info.get('duration_frames', 0)} frames)\n"
                    
                    # Show timeline of appearances
                    all_timestamps = plate_info.get('all_timestamps', [])
                    if len(all_timestamps) <= 10:
                        # Show all timestamps if 10 or fewer
                        status_msg += f"- **All Appearances:** {', '.join(all_timestamps)}\n"
                    else:
                        # Show first 3, last 3, and total count
                        status_msg += f"- **Appearances:** {', '.join(all_timestamps[:3])} ... {', '.join(all_timestamps[-3:])} ({len(all_timestamps)} total)\n"
...
def create_ui():
    ...
    with gr.Blocks(title="LPR System", theme=gr.themes.Soft()) as demo:
        with gr.Row():
            gr.Markdown("# 🚗 License Plate Recognition (UK) - Vehicle Timeline Tracker")
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                process_btn = gr.Button("🔍 Detect & Track", variant="primary", size="lg")
            
            with gr.Column(scale=2):
                status_output = gr.Markdown("*Ready - Now tracking vehicle appearances over time!*")
