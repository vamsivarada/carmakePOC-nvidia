...
class LPRDetector:
    ...
    def osd_sink_pad_buffer_probe(self, pad, info, u_data):
        ...
        while l_frame is not None:
            ...
            # First pass: collect vehicle info, including make
            vehicles = {}
            ...
            while l_obj is not None:
                ...
                if obj_meta.unique_component_id == 1:
                    vehicle_info = {
                        ...
                        "make": "Unknown" # << MODIFIED: Default make
                    }

                    # << NEW: Check for classifier metadata from the new make detector (unique_id=4)
            ...
            while l_obj is not None:
                ...
                if obj_meta.unique_component_id == 2:
                    ...
                    # << MODIFIED: Get vehicle info, which now includes make
                    vehicle_info = vehicles.get(parent_id, {"type": "Unknown", "make": "Unknown"})
                                    ...
                                    if plate_text and plate_text.strip():
                                        ...
                                        if plate not in self.plate_detections:
                                            self.plate_detections[plate] = {
                                                ...
                                                "vehicle_make": vehicle_info["make"], # << NEW
                                                "confidence": label_info.result_prob,
                                                "count": 1
                                            }
                                            print(f"\n🚗 NEW PLATE: {plate} | Vehicle: {vehicle_info['make']} {vehicle_info['type']} | Confidence: {label_info.result_prob:.2f}")
                                        else:
    ...
    def save_results(self):
        ...
        for plate, info in self.plate_detections.items():
            self.results["summary"]["plates"].append({
                ...
                "vehicle_make": info["vehicle_make"], # << NEW
        ...
        # << MODIFIED: Updated print format to include make
        for plate, info in sorted(self.plate_detections.items(), key=lambda x: x[1]['count'], reverse=True):
            print(f"  📋 {plate:15s} | Make: {info['vehicle_make']:12s} | Type: {info['vehicle_type']:10s} | "
    ...
    def run(self):
        ...
        sgie3 = Gst.ElementFactory.make("nvinfer", "secondary-nvinference-engine3") # << NEW
        nvvidconv = Gst.ElementFactory.make("nvvideoconvert", "convertor")
        nvosd = Gst.ElementFactory.make("nvdsosd", "onscreendisplay")
        sink = Gst.ElementFactory.make("fakesink", "fake-sink")

        if not streammux or not pgie or not sgie1 or not sgie2 or not sgie3 or not nvvidconv or not nvosd or not sink: # << MODIFIED
        ...
        # << NEW: Set the config file for the new vehicle make model
        sgie3.set_property('config-file-path',
    "/opt/nvidia/deepstream/deepstream-8.0/sources/apps/sample_apps/deepstream_tao_apps/configs/nvinfer/vehiclemakenet_tao/sgie_vehiclemakenet_onnx_config.txt")
        ...
        # << MODIFIED: Link sgie3 into the pipeline
        streammux.link(pgie)
        pgie.link(sgie1)
        sgie1.link(sgie2)
        sgie2.link(sgie3)
...
def main():
    parser = argparse.ArgumentParser(description='DeepStream License Plate Recognition with Vehicle Make Detection') # << MODIFIED
